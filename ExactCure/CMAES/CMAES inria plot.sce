...
// columns are generation feval something y-values
  ...
  for name = names
    ...
    if err ~= 0
      warning('File ' + [name_prefix + name + name_extension] +' could not be opened');
      data(name) = []; 
    else
...
// data.fit(:,6) = data.fit(:,6) - 2e5; 
    ...
    if or(idxpos) // if any entry in idxpos is true
      plot(data.fit(idxpos,2), log10(0e-49 + abs(data.fit(idxpos,6))), 'b.');
    end

    if or(idxneg)
      plot(data.fit(idxneg,2),log10(0e-49 + abs(data.fit(idxneg,6))), 'k.');
    end
  ...
  if ~isempty(data(name))
    ...
    if Ngeno < 100
      ...
      plot([data(name)($,2); xlast]',[data(name)($, 6:$) ; yrange(1) + (idx2')*(yrange(2)-yrange(1))/(Ngeno-1)]);
	...
	xstring(xlast, yrange(1) + (yrange(2)-yrange(1))*(idx2(i)/(Ngeno-1) - 0.3/max(10,Ngeno)), [str string((i))]); 
      end
    end // annotation
  ...
  if ~isempty(data.stddev)
    ...
    if Ngeno < 100
      ...
      plot([data.stddev($,2); xlast]',[log10(data.stddev($,6:$));yrange(1) + (yrange(2)-yrange(1))*(idx2/(Ngeno-1))]);
	...
	xstring(xlast, yrange(1) + (yrange(2)-yrange(1)) * (idx2(i)/(Ngeno-1) - 0.3/max(Ngeno,10)), [str string(idxvars(i))]); 
      end
    end
