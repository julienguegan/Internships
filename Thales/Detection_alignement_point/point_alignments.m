...
function [point_in_align,align_m] = point_alignments(point)
point_in_align = [];
align_m = [];
% domain
X = 1000;
...
align = find_alignments(point_ext,N,Next,X,Y);
Na = length(align)/8;
disp(['find_alignments : ', num2str(Na)]);
disp(['  t1 : ', num2str(toc)]);
% reduce redundancy
align_m = [];
if( Na>=1 )
    align_m = masking(point_ext,N,Next,align,Na,X,Y);
end

Nm = size(align_m,2);
disp(['masking : ', num2str(Nm)]);
disp(['  t2 : ', num2str(toc)]);
if (Nm>0)
    nn = 0;
