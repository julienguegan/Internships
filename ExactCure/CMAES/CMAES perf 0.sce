...
function [xmin,counteval,time] = purecmaes(f, x0, s0,itermax, tol)//CMA-ES
    timer()
    // --------------------  Initialization --------------------------------  
...
for i = 1:10:100
    x0 = i*ones(20,1)
    [x c t] = purecmaes(frastrigin, x0, 0.5, 100000, 10^-8)
...
xmin = []
for s0 = 0.01:1:10
    [x c t] = purecmaes(frastrigin, 10*ones(20,1), s0, 100000, 10^-8)
    counteval = [counteval c]
    time = [time t]
    xmin = [xmin x]
end
