...
function align = find_alignments_for_csmprt(point, N, Next, X, Y, length_min, thick_max)
align = [];
n_align = 0;
length_max = length_min+round(length_min/10)+1;
max_w_over_len = length_max/thick_max-1;
...
for i = 1:N-1
    for j = i+1:N
        ...
        if( len == 0 || len<length_min || len>length_max)
            continue;
        end
        dx = dx/len ;		dy = dy/len;
